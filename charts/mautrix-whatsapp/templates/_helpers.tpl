...
app: {{ include "mautrix-whatsapp.name" . }}
{{- end -}}

{{- define "mautrix-whatsapp.componentSelectorLabels" -}}
{{ include "mautrix-whatsapp.selectorLabels" .context }}
...
{{- define "mautrix-whatsapp.componentLabels" -}}
{{ include "mautrix-whatsapp.labels" .context }}
app.kubernetes.io/component: {{ .component }}
{{- end -}}

{{- define "mautrix-whatsapp.image" -}}
{{- if .Values.image.tag -}}
...
{{- define "mautrix-whatsapp.postgresImage" -}}
...
{{- define "mautrix-whatsapp.configSecretName" -}}
{{- printf "%s-config" (include "mautrix-whatsapp.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "mautrix-whatsapp.registrationConfigMapName" -}}
{{- if .Values.registration.synapseConfigMapName -}}
{{- .Values.registration.synapseConfigMapName | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-registration" (include "mautrix-whatsapp.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end -}}
...
{{- define "mautrix-whatsapp.homeserverDomain" -}}
{{- required "values.homeserver.domain is required (example: matrix.example.com)" .Values.homeserver.domain -}}
{{- end -}}

{{- define "mautrix-whatsapp.postgresFullname" -}}
{{- printf "%s-postgres" (include "mautrix-whatsapp.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "mautrix-whatsapp.databaseConnectionString" -}}
{{- if .Values.database.connectionString -}}
{{- .Values.database.connectionString -}}
{{- else if .Values.postgres.enabled -}}
{{- printf "postgres://%s:%s@%s:%v/%s" .Values.database.user .Values.database.password (include "mautrix-whatsapp.postgresFullname" .) .Values.postgres.service.port .Values.database.name -}}
{{- else -}}
{{- required "values.database.connectionString is required when postgres.enabled=false" .Values.database.connectionString -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-whatsapp.appserviceAddress" -}}
{{- if .Values.appservice.address -}}
{{- .Values.appservice.address -}}
{{- else -}}
{{- printf "http://%s.%s.svc.cluster.local:%v" (include "mautrix-whatsapp.fullname" .) .Release.Namespace .Values.service.port -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-whatsapp.registrationServiceUrl" -}}
{{- if .Values.registration.serviceUrl -}}
{{- .Values.registration.serviceUrl -}}
{{- else -}}
{{- include "mautrix-whatsapp.appserviceAddress" . -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-whatsapp.runtimeSecretName" -}}
...
{{- else -}}
{{- printf "%s-runtime-secrets" (include "mautrix-whatsapp.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-whatsapp.ensureRuntimeSecrets" -}}
...
{{- $secretName := include "mautrix-whatsapp.runtimeSecretName" . -}}
...
{{- define "mautrix-whatsapp.registrationAsToken" -}}
{{- include "mautrix-whatsapp.ensureRuntimeSecrets" . -}}
{{- index (index .Values.registration "_computedRuntimeSecrets") "asToken" -}}
{{- end -}}

{{- define "mautrix-whatsapp.registrationHsToken" -}}
{{- include "mautrix-whatsapp.ensureRuntimeSecrets" . -}}
{{- index (index .Values.registration "_computedRuntimeSecrets") "hsToken" -}}
{{- end -}}

{{- define "mautrix-whatsapp.provisioningSharedSecret" -}}
{{- include "mautrix-whatsapp.ensureRuntimeSecrets" . -}}
{{- index (index .Values.registration "_computedRuntimeSecrets") "provisioningSharedSecret" -}}
{{- end -}}

{{- define "mautrix-whatsapp.registrationSenderLocalpart" -}}
...
{{- define "mautrix-whatsapp.renderTemplateRegex" -}}
{{- $template := .template -}}
{{- $field := .field -}}
{{- $normalized := include "mautrix-whatsapp.normalizeBridgeTemplate" (dict "template" $template "field" $field "legacyReplacement" "{placeholder}") -}}
{{- regexReplaceAll "\\{[A-Za-z_][A-Za-z0-9_]*\\}" $normalized ".*" -}}
{{- end -}}

{{- define "mautrix-whatsapp.normalizeBridgeTemplate" -}}
{{- $template := .template -}}
{{- $field := .field -}}
...
{{- define "mautrix-whatsapp.registrationUserRegex" -}}
{{- if .Values.registration.userRegex -}}
{{- .Values.registration.userRegex -}}
{{- else -}}
{{- printf "@%s:%s" (include "mautrix-whatsapp.renderTemplateRegex" (dict "template" .Values.bridge.usernameTemplate "field" "bridge.usernameTemplate")) (include "mautrix-whatsapp.homeserverDomain" .) -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-whatsapp.registrationAliasRegex" -}}
{{- if .Values.registration.aliasRegex -}}
{{- .Values.registration.aliasRegex -}}
{{- else -}}
{{- printf "#%s:%s" (include "mautrix-whatsapp.renderTemplateRegex" (dict "template" .Values.bridge.aliasTemplate "field" "bridge.aliasTemplate")) (include "mautrix-whatsapp.homeserverDomain" .) -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-whatsapp.registrationConfig" -}}
id: {{ .Values.appservice.id | quote }}
url: {{ include "mautrix-whatsapp.registrationServiceUrl" . | quote }}
as_token: {{ include "mautrix-whatsapp.registrationAsToken" . | quote }}
hs_token: {{ include "mautrix-whatsapp.registrationHsToken" . | quote }}
sender_localpart: {{ include "mautrix-whatsapp.registrationSenderLocalpart" . | quote }}
rate_limited: {{ .Values.registration.rateLimited }}
de.sorunome.msc2409.push_ephemeral: {{ .Values.appservice.ephemeralEvents }}
namespaces:
  users:
    - exclusive: true
      regex: {{ include "mautrix-whatsapp.registrationUserRegex" . | squote }}
  aliases:
    - exclusive: true
      regex: {{ include "mautrix-whatsapp.registrationAliasRegex" . | squote }}
{{- end -}}
