...
A Matrix-Discord puppeting bridge. See [mautrix/discord](https://github.com/mautrix/discord) for details.

## Overview

This chart deploys `mautrix-discord` with:
...
```bash
helm upgrade --install mautrix-discord ./charts/mautrix-discord -f mautrix-discord-values.yaml
```
...
helm repo update
helm upgrade --install mautrix-discord matrix-helm-charts/mautrix-discord -n mautrix-discord --create-namespace --values mautrix-discord-values.yaml
```
...
- ConfigMap: `<release>-mautrix-discord-registration`
- Key: `appservice-registration-discord.yaml`
...
synapse:
  appservices:
    - configMap: <release>-mautrix-discord-registration
      configMapKey: appservice-registration-discord.yaml
```
...
- from chart-managed Secret (default `<release>-mautrix-discord-runtime-secrets`) when it already exists
...
database:
  connectionString: postgres://mautrix_discord:replace_me@postgres.example.com:5432/mautrix_discord
```
...
- `values.matrix.example.yaml`: recommended Matrix/ESS-focused mautrix-discord config example.
...
```bash
helm lint ./charts/mautrix-discord -f ./charts/mautrix-discord/values.example.yaml
```

## Verify

```bash
kubectl get pods,svc -l app.kubernetes.io/instance=mautrix-discord -n discord
kubectl get configmap <release>-mautrix-discord-registration -n discord
kubectl get configmap <release>-mautrix-discord-registration -n <synapse-namespace>
```

## Docs

- [Bridge setup with Docker](https://docs.mau.fi/bridges/general/docker-setup.html?bridge=discord)
- [Initial bridge config](https://docs.mau.fi/bridges/general/initial-config.html#mautrix-discord)
- [Registering appservices](https://docs.mau.fi/bridges/general/registering-appservices.html)
- [mautrix-discord repository](https://github.com/mautrix/discord)
