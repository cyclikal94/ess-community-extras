...
app: {{ include "mautrix-discord.name" . }}
{{- end -}}

{{- define "mautrix-discord.componentSelectorLabels" -}}
{{ include "mautrix-discord.selectorLabels" .context }}
...
{{- define "mautrix-discord.componentLabels" -}}
{{ include "mautrix-discord.labels" .context }}
app.kubernetes.io/component: {{ .component }}
{{- end -}}

{{- define "mautrix-discord.image" -}}
{{- if .Values.image.tag -}}
...
{{- define "mautrix-discord.postgresImage" -}}
...
{{- define "mautrix-discord.configSecretName" -}}
{{- printf "%s-config" (include "mautrix-discord.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "mautrix-discord.registrationConfigMapName" -}}
{{- if .Values.registration.synapseConfigMapName -}}
{{- .Values.registration.synapseConfigMapName | trunc 63 | trimSuffix "-" -}}
{{- else -}}
{{- printf "%s-registration" (include "mautrix-discord.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.registrationFileKey" -}}
appservice-registration-discord.yaml
{{- end -}}

{{- define "mautrix-discord.homeserverDomain" -}}
{{- required "values.homeserver.domain is required (example: matrix.example.com)" .Values.homeserver.domain -}}
{{- end -}}

{{- define "mautrix-discord.postgresFullname" -}}
{{- printf "%s-postgres" (include "mautrix-discord.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}

{{- define "mautrix-discord.databaseConnectionString" -}}
{{- if .Values.database.connectionString -}}
{{- .Values.database.connectionString -}}
{{- else if .Values.postgres.enabled -}}
{{- printf "postgres://%s:%s@%s:%v/%s" .Values.database.user .Values.database.password (include "mautrix-discord.postgresFullname" .) .Values.postgres.service.port .Values.database.name -}}
{{- else -}}
{{- required "values.database.connectionString is required when postgres.enabled=false" .Values.database.connectionString -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.appserviceAddress" -}}
{{- if .Values.appservice.address -}}
{{- .Values.appservice.address -}}
{{- else -}}
{{- printf "http://%s.%s.svc.cluster.local:%v" (include "mautrix-discord.fullname" .) .Release.Namespace .Values.service.port -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.registrationServiceUrl" -}}
{{- if .Values.registration.serviceUrl -}}
{{- .Values.registration.serviceUrl -}}
{{- else -}}
{{- include "mautrix-discord.appserviceAddress" . -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.runtimeSecretName" -}}
...
{{- else -}}
{{- printf "%s-runtime-secrets" (include "mautrix-discord.fullname" .) | trunc 63 | trimSuffix "-" -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.ensureRuntimeSecrets" -}}
...
{{- $secretName := include "mautrix-discord.runtimeSecretName" . -}}
...
{{- define "mautrix-discord.registrationAsToken" -}}
{{- include "mautrix-discord.ensureRuntimeSecrets" . -}}
{{- index (index .Values.registration "_computedRuntimeSecrets") "asToken" -}}
{{- end -}}

{{- define "mautrix-discord.registrationHsToken" -}}
{{- include "mautrix-discord.ensureRuntimeSecrets" . -}}
{{- index (index .Values.registration "_computedRuntimeSecrets") "hsToken" -}}
{{- end -}}

{{- define "mautrix-discord.provisioningSharedSecret" -}}
{{- include "mautrix-discord.ensureRuntimeSecrets" . -}}
{{- index (index .Values.registration "_computedRuntimeSecrets") "provisioningSharedSecret" -}}
{{- end -}}

{{- define "mautrix-discord.registrationSenderLocalpart" -}}
...
{{- define "mautrix-discord.renderTemplateRegex" -}}
{{- $template := .template -}}
{{- $field := .field -}}
{{- $normalized := include "mautrix-discord.normalizeBridgeTemplate" (dict "template" $template "field" $field "legacyReplacement" "{placeholder}") -}}
{{- regexReplaceAll "\\{[A-Za-z_][A-Za-z0-9_]*\\}" $normalized ".*" -}}
{{- end -}}

{{- define "mautrix-discord.normalizeBridgeTemplate" -}}
{{- $template := .template -}}
{{- $field := .field -}}
...
{{- define "mautrix-discord.registrationUserRegex" -}}
{{- if .Values.registration.userRegex -}}
{{- .Values.registration.userRegex -}}
{{- else -}}
{{- printf "@%s:%s" (include "mautrix-discord.renderTemplateRegex" (dict "template" .Values.bridge.usernameTemplate "field" "bridge.usernameTemplate")) (include "mautrix-discord.homeserverDomain" .) -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.registrationAliasRegex" -}}
{{- if .Values.registration.aliasRegex -}}
{{- .Values.registration.aliasRegex -}}
{{- else -}}
{{- printf "#%s:%s" (include "mautrix-discord.renderTemplateRegex" (dict "template" .Values.bridge.aliasTemplate "field" "bridge.aliasTemplate")) (include "mautrix-discord.homeserverDomain" .) -}}
{{- end -}}
{{- end -}}

{{- define "mautrix-discord.registrationConfig" -}}
id: {{ .Values.appservice.id | quote }}
url: {{ include "mautrix-discord.registrationServiceUrl" . | quote }}
as_token: {{ include "mautrix-discord.registrationAsToken" . | quote }}
hs_token: {{ include "mautrix-discord.registrationHsToken" . | quote }}
sender_localpart: {{ include "mautrix-discord.registrationSenderLocalpart" . | quote }}
rate_limited: {{ .Values.registration.rateLimited }}
de.sorunome.msc2409.push_ephemeral: {{ .Values.appservice.ephemeralEvents }}
namespaces:
  users:
    - exclusive: true
      regex: {{ include "mautrix-discord.registrationUserRegex" . | squote }}
  aliases:
    - exclusive: true
      regex: {{ include "mautrix-discord.registrationAliasRegex" . | squote }}
{{- end -}}
